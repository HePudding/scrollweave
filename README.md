...
![新版编辑器](docs/screenshots/editor-v2.png)
...
打开 http://127.0.0.1:4100。首次启动是空作品。默认作品目录为源码目录旁边的 `ScrollWeaveProjects/My-first-work`，**不是源码目录**。顶部“作品”可新建目录、打开已有目录、打开项目包或旧版 JSON。
...
4. **动画**：单选片段 → 定位播放头 → 属性旁 ◇。已有动画的属性继续在当前位置打帧；“自动关键帧”对新属性也生效。选中属性的关键帧在时间线展开，可拖动、点击修改值/时间/缓动和贝塞尔曲线。
...
轨道由下到上叠放，支持锁定和隐藏。所有人/Agent修改经过相同事务、校验和 revision 冲突检测。撤销最近 80 次事务；重启恢复结构、revision、选择和预览，不恢复撤销栈。

| 快捷键                 | 操作                                  |
| ---------------------- | ------------------------------------- |
| Space                  | 播放 / 暂停                           |
| Ctrl / Command + B     | 播放头处分割全部选中片段              |
| Ctrl / Command + C / V | 复制 / 在播放头粘贴到新轨道           |
